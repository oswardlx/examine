...
local function dealExaminaRequest()
    ...
    local level = request:getNumParam("level", true, true)
    log:debug("identity_id:"..identity_id)
    local result,deal_time,person_name = service.dealExaminaRequest({
        identity_id = identity_id,
        info_id = info_id,
        e_p_id = e_p_id,
        status = status,
    ...
    ngx.say(cjson.encode({ success = result,deal_time=deal_time,person_name=person_name }))
end
