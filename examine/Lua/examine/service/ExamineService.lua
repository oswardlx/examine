...
--      local person_name=""
      ...
      if status ==2 then
          func = function()
              flag,deal_id=dao.dealExaminaRequest(param)
      ...
      local status = dao.txFunction(func)
      local deal_time = dao.getDealTime(deal_id)
      local person_name = dao.getPersonName(param.e_p_id,param.identity_id)
        log:debug(person_name)
--        log:debug("status:"..status.."  deal_time:"..deal_time.."  person_name:"..person_name)
      return status,deal_time,person_name
end
