...
local filename = os.date("examine%Y%m%d", os.time())
...
local function findMoneyBySchoolTypeList(stage_type, time_index, start_time, end_time, search_str, page_size, offset, status, office_id)
    log:debug("111111111111111111111111111111:" .. office_id)
    local addsql1 = ""
    local addsql2 = ""
    local addsql3 = ""
    local addsql4 = ""
...
function _M.getDealTime(deal_id)
    local sql = "SELECT  create_time  FROM t_social_examine_status where id = "..deal_id
    log:debug(sql)
    local db = DBUtil:getDb();
    local result = db:query(sql)
    if not result or TableUtil:length(result) == 0 then
    ...
    return result[1]['create_time']
end

function _M.getPersonName(e_p_id,identity_id)
    local sql = "SELECT person_name from t_social_examine_person where person_id = "..e_p_id.." and identity_id = "..identity_id
    log:debug(sql)
    local db = DBUtil:getDb();
    local result = db:query(sql)
    if not result or TableUtil:length(result) == 0 then
